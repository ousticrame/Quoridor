...
def student_project_allocation_tool(
    ...
    allocation : Optional[Dict[int, int]] =  student_project_allocation(students, projects, preferences, project_capacities, [])
    ...
    student_nodes = [node for node, data in G.nodes(data=True) if data.get("type") == "student"]
    project_nodes = [node for node, data in G.nodes(data=True) if data.get("type") == "project"]

    plt.figure(figsize=(10, 8))  # Increase figure size for better readability
    nx.draw_networkx_nodes(G, pos, nodelist=student_nodes, node_color="skyblue", node_size=800)
    nx.draw_networkx_nodes(G, pos, nodelist=project_nodes, node_color="lightgreen", node_size=1200)
